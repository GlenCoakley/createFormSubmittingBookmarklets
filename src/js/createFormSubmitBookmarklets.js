[200~function buildBookmark(form) {
  function getFormParams(form) {
    function addParam(name, value, out) {  return out + (out ? ',' : '') + '"' + name + '":"' + value + '"' };
    function addAll(form, tag, out) {
      for (var els of form.getElementsByTagName(tag)) { 
        out = addParam (els.name, els.value, out);
      }
      return out
    }
    var out = ''
    for (var tag of ['input', 'textarea', 'submit']) {
      out = addAll(form, tag, out);
    }
    var params = '';
    for (var sels of form.getElementsByTagName('select')) {
      for (var opts of form.getElementsByTagName('option')) {
  ...
  return header + 'javascript:function sf(ur,ty,fd){' +
    ...
    'const fm=me("form",{action:ur,method:ty,style:"display:hidden;"});' +
...
try {
  var bookmarks = '';
  for (var form of window.document.getElementsByTagName('form')) {
    bookmarks += buildBookmark(form) + '\r\n';
  }
} catch (e) { console.trace(e);alert(e) }
var winHndl  = open('', 'bbout', "width=800,height=600,resizable,scrollbars");
winHndl.document.write('<html><title>Form Bookmarklets</title><body>' + 
  bookmarks + '</body></html>');
